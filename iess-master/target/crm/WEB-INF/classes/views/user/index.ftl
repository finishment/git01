            ...
            <#--            <%-- 设置分页导航 --%>-->
                <nav style="text-align: center">
                    <ul class="pagination  center">
                        ...
                        <#if page.pageNum!=1>
                            <li>
                                <a href="${ctx}/user/indexx?pageNum=${page.prePage}&title=${title!""}&date=${date!""}&typeId=${typeId!""}"><span>«</span> </a>
                            </li>
                        </#if>
                        <#--                    <%-- 导航页数 --%>-->
                        <#list page.startNavPage..page.endNavPage as p>
                            <li <#if page.pageNum == p>class="active"</#if> >
                                <a href="${ctx}/user/indexx?pageNum=${p}&title=${title!""}&date=${date!""}&typeId=${typeId!""}">${p}</a>
                            </li>
                        </#list>
                        <#--                    <%-- 如果当前不是最后一页，则显示下一页的按钮 --%>-->
                        <#if page.pageNum < page.totalPages>
                            <li>
                                <a href="${ctx}/user/index?&pageNum=${page.nextPage}&title=${title!""}&date=${date!""}&typeId=${typeId!""}">
                                    <span>»</span>
                                </a>
                            </li>
                        </#if>
...
<script type="text/javascript">
    $("#search_btn").click(function () {
        ...
        window.location.href=ctx+"/user/indexx?title="+title;
    });
    $("#btn1").click(function () {
        var btn1 = $("#btn1").val();
