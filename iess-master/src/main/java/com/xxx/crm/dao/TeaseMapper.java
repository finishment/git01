...
public interface TeaseMapper {
    ...
    @Insert("insert into iess.t_tease(userId,title, content, typeId, pubTime, lon, lat)VALUES (#{userId},#{title}, #{content}, #{typeId},now(), #{lon}, #{lat})")
    int addNote(Note note);

    @Delete("delete from iess.t_tease where noteId=#{noteId}")
    int deleteNote(int noteId);

    @Update("update iess.t_tease " +
            "set title=#{title},content=#{content},typeId=#{typeId},lon=#{lon},lat=#{lat} where noteId=#{noteId}")
    int updateNote(Note note);

    @Select("select count(*) from iess.t_tease where typeId=#{typeId}")
    int selectNotesByID(int typeId);

    @Select("SELECT count(1) noteCount,DATE_FORMAT(pubTime,'%Y年%m月') groupName FROM iess.t_tease n " +
    ...
    @Select("SELECT count(noteId) noteCount, t.typeId, typeName groupName FROM iess.t_tease n " +
    ...
    @Select("select Lon,Lat from iess.t_tease tn inner join iess.t_note_type tnt on tnt.typeId = tn.typeId where tnt.userId =#{userId} ")
    List<Note> queryNoteList(Integer userId);

    @Select("select count(*) from iess.t_tease")
